...
function handlePaymentAttentionChange(event) {
  paymentAttentionCount.value = event.detail.count;
}

onMounted(() => {
  window.addEventListener('app:page-change', handlePageChange);
  window.addEventListener('app:payment-attention-change', handlePaymentAttentionChange);
});

onBeforeUnmount(() => {
  window.removeEventListener('app:page-change', handlePageChange);
  window.removeEventListener('app:payment-attention-change', handlePaymentAttentionChange);
});
</script>
...
<template>
  <button
    v-for="page in pages"
    :key="page.name"
    :class="{ active: activePage === page.name, 'nav-secondary': page.secondary }"
    :data-page="page.name"
    :title="page.title"
    :aria-label="page.title"
  >
    <span class="ico">
      <!-- The SVG markup is a closed, local constant and never contains user data. -->
      <!-- eslint-disable-next-line vue/no-v-html -->
      <svg viewBox="0 0 24 24" aria-hidden="true" v-html="page.icon"></svg>
    </span>
    <span class="nav-label">{{ page.label }}</span>
    <span
      v-if="page.name === 'payments'"
      id="paymentNavBadge"
      class="nav-badge"
      :class="{ show: paymentAttentionCount > 0 }"
      >{{ paymentAttentionCount || '' }}</span
    >
  </button>
</template>

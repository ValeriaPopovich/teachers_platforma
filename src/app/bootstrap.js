...
const dashboardView = createDashboardView({ store, openLesson: (id) => scheduleView.openLesson(id), retentionDays: RETENTION_DAYS });
...
function exportBackup() {
  downloadJson(`teachers-platforma-${localDay()}.json`, makeBackup(store.getState(), { appVersion: 'refactor-v5-part2' }));
...
$('#onboardingForm')?.addEventListener('submit',(event)=>{event.preventDefault();const name=new FormData(event.currentTarget).get('tutor')?.trim();if(!name)return;settingsService.completeOnboarding(name);modal.closeAll();toast('Профиль настроен');});

const sumPaymentAmounts = (payments) =>
...
export function finances(data, id) {
  ...
  if (student?.payType === 'package') {
    ...
    const bought =
      ...
      packagePayments.reduce((sum, payment) => sum + (+payment.packageLessons || 0), 0);
    const used =
      (+archive.packageUsed || 0) + done.filter((lesson) => lesson.payment === 'package').length;
    const extraDebt =

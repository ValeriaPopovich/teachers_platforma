...
export const BUILTIN_GOALS = [
  'Повысить успеваемость', 'Подготовиться к ОГЭ', 'Подготовиться к ВПР', 'Устранить пробелы',
  'Подготовиться к контрольной', 'Углубить знания', 'Поступить в профильный класс',
];

export function slotRow(slot = { day: 1, time: '17:00' }) {
  const row = document.createElement('div');
  row.className = 'schedule-slot';
  const days = [['Понедельник', 1], ['Вторник', 2], ['Среда', 3], ['Четверг', 4], ['Пятница', 5], ['Суббота', 6], ['Воскресенье', 0]];
...
export function setSlots(target, slots = [], ensureRow = true) {
  ...
  if (!element) return;
  element.innerHTML = '';
  (slots.length ? slots : ensureRow ? [{ day: 1, time: '17:00' }] : []).forEach((slot) => element.append(slotRow(slot)));
}

export function getSlots(target) {
  return $$(target + ' .schedule-slot').map((row) => ({ day: +$('.slot-day', row).value, time: $('.slot-time', row).value })).filter((slot) => slot.time);
}
...
function syncSelectableChips(container, itemSelector) {
  ...
  items.filter((item) => $('input[type=checkbox]', item)?.checked).forEach((item) => box.append(item));
  items.filter((item) => !$('input[type=checkbox]', item)?.checked).forEach((item) => box.append(item));
}
...
export function createStudentFormView({ store, service, modal, dialog, toast }) {
  ...
  function renderGoalOptions(selected = []) {
    ...
    const unknown = selected.filter((goal) => !BUILTIN_GOALS.includes(goal) && !(state.settings.customGoals || []).includes(goal));
    const custom = [...new Set([...(state.settings.customGoals || []), ...unknown])];
    const all = [...BUILTIN_GOALS.filter((goal) => !deleted.has(goal)), ...custom];
    $('#goalChips').innerHTML = all.map((goal) => `<span class="goal-option" data-goal-option="${escapeHtml(goal)}"><label class="goal-chip"><input type="checkbox" value="${escapeHtml(goal)}">${escapeHtml(goal)}</label><button type="button" class="goal-remove" data-remove-goal="${escapeHtml(goal)}" title="Удалить цель из списка" aria-label="Удалить цель из списка">×</button></span>`).join('');
  }

  function setGoalChecks(goals = '') {
    const selected = String(goals).split(',').map((goal) => goal.trim()).filter(Boolean);
    renderGoalOptions(selected);
    $$('#goalChips input').forEach((input) => { input.checked = selected.includes(input.value); });
    syncSelectableChips('#goalChips', '.goal-option');
    $('#customGoal').value = '';
  }

  function collectGoals() {
    return [...$$('#goalChips input:checked').map((input) => input.value), ...$('#customGoal').value.split(',').map((goal) => goal.trim()).filter(Boolean)].join(', ');
  }

  function syncPackageField() {
    const enabled = form.elements.payType.value === 'package';
    const current = form.elements.id.value ? getStudentById(store.getState(), form.elements.id.value) : null;
    const startInput = form.elements.billingStartDate;
    const selectedStart = startInput.value
      ? startInput.value === localDay() ? Date.now() : new Date(`${startInput.value}T00:00`).getTime()
      : Date.now();
    const start = current ? Math.max(+current.createdAt || 0, +current.billingSince || 0) : selectedStart;
    ...
    if (hint && !current) hint.textContent = slots.length ? `${count} ${lessonCountWord(count)} · ${money(count * (+form.elements.price.value || 0))}` : 'Добавьте регулярное расписание — сумма рассчитается автоматически';
  }

  function slotWarnings(input) {
    const own = ownSlotConflict(input.scheduleSlots, input.duration);
    const conflicts = recurringConflicts(store.getState(), input.scheduleSlots, input.duration, 'student', input.id);
    return [own ? `занятия ${own} пересекаются между собой` : '', conflicts.length ? `время пересекается с: ${conflicts.join(', ')}` : ''].filter(Boolean);
  }
  ...
  async function submit(event) {
    ...
    if (!result.ok && result.code === 'PAYMENT_FORMAT_CONFIRM_REQUIRED') {
      const confirmed = await dialog.ask(result.message, 'Сменить формат оплаты?', 'Сменить формат');
      if (!confirmed) return;
    ...
    modal.closeAll();
    form.reset();
    toast(warnings.length ? `Сохранено. Внимание: ${warnings.join('; ')}` : result.formatChanged ? 'Формат оплаты изменён, новый баланс начат' : 'Ученик и расписание сохранены');
  }
  ...
  $('#addStudentSlot').addEventListener('click', () => { $('#studentScheduleSlots').append(slotRow()); syncPackageField(); });
  ...
  $('#studentModal').addEventListener('change', (event) => {
    if (event.target.matches('#goalChips input[type=checkbox]')) syncSelectableChips('#goalChips', '.goal-option');
  });
  $('#customGoal').addEventListener('keydown', (event) => {
    ...
    const existing = $$('#goalChips input').find((input) => input.value.toLowerCase() === value.toLowerCase());
  ...
  $('#studentModal').addEventListener('click', async (event) => {
    ...
    if (!(await dialog.ask(`Удалить цель «${goal}» из списка? Она также будет убрана у учеников, которым назначена.`, 'Удаление цели', 'Удалить'))) return;

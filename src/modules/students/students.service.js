...
export function createStudentsService({ store, uid, now = () => Date.now() }) {
  function saveStudent(input, options = {}) {
    const current = input.id ? store.getState().students.find((student) => student.id === input.id) : null;
    const name = String(input.name || '').trim();
    if (!name) return { ok: false, code: 'NAME_REQUIRED', message: 'Введите имя ученика, а не только пробелы.' };
    const formatChanged = !!current && current.payType !== input.payType;
    if (formatChanged && !options.confirmPaymentFormatChange) {
      return { ok: false, code: 'PAYMENT_FORMAT_CONFIRM_REQUIRED', message: 'Старые занятия и платежи останутся в истории, но новый баланс начнёт считаться с момента смены формата.' };
    }
    const at = now();
    const selectedStart = input.billingStartDate && input.billingStartDate !== options.today
      ? new Date(`${input.billingStartDate}T00:00`).getTime()
      : at;
    const startedAt = current && !formatChanged
      ? Math.max(+current.createdAt || 0, +current.billingSince || 0)
      : current ? at : selectedStart;
    const student = {
      ...input,
      name,
      price: +input.price || 0,
    ...
    student.packageSize = student.payType === 'package'
    ...
    store.update(current ? 'students:update' : 'students:create', (draft) => {
      if (formatChanged) draft.financeArchive[student.id] = { packageUsed: 0, singleCharged: 0, since: student.billingSince };
      if (current) {
      ...
      let next = generateSchedule(draft, { type: 'student', id: student.id, slots: student.scheduleSlots, uid });
      const savedStudent = next.students.find((item) => item.id === student.id);
      next.lessons
        .filter((lesson) => !lesson.groupId && lesson.studentId === student.id && lesson.status === 'planned' && new Date(lesson.date).getTime() >= at)
        .forEach((lesson) => {
          lesson.amount = +savedStudent?.price || 0;
          if (formatChanged && lesson.payment !== 'paid') lesson.payment = savedStudent?.payType === 'package' ? 'package' : 'unpaid';
        });
  ...
  function removeStudent(id) {
    ...
    store.update('students:remove', (draft) => {
      ...
      draft.groups.forEach((group) => { group.members = (group.members || []).filter((studentId) => studentId !== id); });
      const emptyGroups = new Set(draft.groups.filter((group) => !(group.members || []).length).map((group) => group.id));
  ...
  function saveGroup(input) {
    const name = String(input.name || '').trim();
    if (!name) return { ok: false, code: 'NAME_REQUIRED', message: 'Введите название группы, а не только пробелы.' };
    const members = [...new Set(input.members || [])];
    if (!members.length) return { ok: false, code: 'MEMBERS_REQUIRED', message: 'Выберите хотя бы одного ученика.' };
    const current = input.id ? store.getState().groups.find((group) => group.id === input.id) : null;
    const group = { ...input, name, members, duration: +input.duration || 60, scheduleSlots: input.scheduleSlots || [], id: current?.id || uid() };
    store.update(current ? 'groups:update' : 'groups:create', (draft) => {
      ...
      let next = generateSchedule(draft, { type: 'group', id: group.id, slots: group.scheduleSlots, uid });
      next.lessons = next.lessons.filter((lesson) => !(lesson.groupId === group.id && lesson.status === 'planned' && new Date(lesson.date) >= new Date() && !group.members.includes(lesson.studentId)));
      next = syncFutureGroupBilling(next).data;
      replaceDraft(draft, next);
    });
  ...
  function removeGroup(id) {
    ...
    store.update('groups:remove', (draft) => {
      const lessonIds = new Set(draft.lessons.filter((lesson) => lesson.groupId === id).map((lesson) => lesson.id));
  ...
  function addCustomGoal(goal) {
    ...
    store.update('students:goal-add', (draft) => {
      ...
      draft.settings.deletedGoals = (draft.settings.deletedGoals || []).filter((item) => item.toLowerCase() !== value.toLowerCase());
    });
  ...
  function removeGoal(goal, builtinGoals = []) {
    store.update('students:goal-remove', (draft) => {
      if (builtinGoals.includes(goal)) draft.settings.deletedGoals = [...new Set([...(draft.settings.deletedGoals || []), goal])];
      draft.settings.customGoals = (draft.settings.customGoals || []).filter((item) => item !== goal);
      draft.students.forEach((student) => {
        student.goals = String(student.goals || '').split(',').map((item) => item.trim()).filter((item) => item && item !== goal).join(', ');
      });
    });
    return { ok: true };
  }

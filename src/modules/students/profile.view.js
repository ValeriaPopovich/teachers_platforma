...
import { getStudentById, getStudentLessons, getStudentMetrics, homeworkGrade, scheduleText } from './students.selectors.js';

function icon(name) {
  const paths = {
    attendance: '<path d="M8.5 12.5a3 3 0 1 0 0-6 3 3 0 0 0 0 6Zm7-1a2.5 2.5 0 1 0 0-5 2.5 2.5 0 0 0 0 5ZM3.5 18.5c0-3 2.2-5 5-5s5 2 5 5M13.5 14.5c2.5 0 4 1.6 4 4"/>',
    homework: '<path d="M5 18.5V14m4 4.5V9m4 9.5V5.5m4 13V11"/>',
    tests: '<path d="M7 3.5h7l4 4v13H7z"/><path d="M14 3.5v4h4M10 12h5m-5 3h5"/>',
    calendar: '<rect x="3.5" y="5.5" width="17" height="15" rx="3"/><path d="M7.5 3v5m9-5v5M3.5 10h17"/>',
...
function statusName(value) {
  return ({ planned: 'Запланировано', unconfirmed: 'Требует подтверждения', done: 'Проведено', missed: 'Пропуск', paid_missed: 'Пропуск с оплатой', moved: 'Перенесено', cancelled: 'Отменено' })[value] || 'Статус не указан';
}
...
export function createProfileView({ store, modal, onEditStudent, onAddLesson, onEditLesson, onDeleteLesson, onDeleteStudent }) {
  ...
  function render(id) {
    ...
    const tests = lessons.filter((lesson) => lesson.status === 'done' && lesson.testDone === 'yes').length;
    const next = state.lessons.filter((lesson) => lesson.studentId === id && lesson.status === 'planned' && new Date(lesson.date) > new Date()).sort((a, b) => new Date(a.date) - new Date(b.date))[0];
    ...
    const lessonPreview = lastLesson ? `<div class="profile-preview-row"><span><small>Дата</small><b>${formatDate(lastLesson.date, true)}</b></span><span><small>Статус</small><b>${statusName(lastLesson.status)}</b></span><span><small>Темы / комментарий</small><b>${escapeHtml(lastLesson.topics || '—')}</b></span><span><small>ДЗ</small><b>${escapeHtml(lastLesson.homework || '—')}</b></span><span><small>Проверочная</small><b>${lastLesson.testDone === 'yes' ? escapeHtml(lastLesson.testName || 'Работа') : '—'}</b></span></div>` : '<p class="profile-empty-history">Проведённых занятий пока нет.</p>';
    ...
    body.innerHTML = `<div class="profile-redesign">
      ...
      <section class="profile-panel profile-panel-history" data-profile-panel="history" hidden><h3 class="profile-history-heading">История занятий</h3>${lessons.length ? `<div style="overflow:auto"><table class="mini-table"><thead><tr><th>Дата</th><th>Статус</th><th>Темы / комментарий</th><th>ДЗ</th><th>Проверочная</th><th aria-label="Действия"></th></tr></thead><tbody>${lessons.slice(0, 20).map(historyRow).join('')}</tbody></table></div>` : '<div class="empty">Занятий пока нет</div>'}${state.topicLog[id]?.length ? `<h3 style="margin-top:18px">Ранее пройденные темы (архив)</h3><ul style="margin:0;padding-left:18px;max-height:220px;overflow:auto">${[...state.topicLog[id]].sort((a, b) => b.d.localeCompare(a.d)).map((entry) => `<li style="margin:3px 0"><b>${formatDate(entry.d)}</b> — ${escapeHtml(entry.t)}</li>`).join('')}</ul>` : ''}</section>
  ...
  function activateTab(name) {
    body.querySelectorAll('[data-profile-panel]').forEach((panel) => { panel.hidden = panel.dataset.profilePanel !== name; });
    body.querySelectorAll('[data-profile-tab]').forEach((button) => button.setAttribute('aria-selected', String(button.dataset.profileTab === name)));
  }
  ...
  body?.addEventListener('click', async (event) => {
    ...
    const deleteId = event.target.closest('[data-delete-history-lesson]')?.dataset.deleteHistoryLesson;
  ...
  body?.addEventListener('keydown', (event) => {
    ...
    if (id) { event.preventDefault(); onEditLesson?.(id); }
  });

...
export function createStudentsView({ store, service, modal, dialog, toast, schedule }) {
  ...
  const profile = createProfileView({
    ...
    onDeleteStudent: async (id) => {
      ...
      if (!(await dialog.ask(`Удалить ученика «${student.name}» вместе с его занятиями и платежами?`, 'Удаление ученика', 'Удалить'))) return false;
  ...
  function renderStudents() {
    ...
    const list = state.students.filter((student) => {
      const haystack = `${student.name || ''} ${student.grade || ''} ${student.contact || ''} ${student.parentContact || ''}`.toLowerCase();
    ...
    if (!grid) return;
    grid.innerHTML = list.length ? list.map((student) => {
      const metric = getStudentMetrics(state, student.id);
      const finance = finances(state, student.id);
      const payment = student.payType === 'package'
        ? `${finance.balanceLessons ?? 0} зан. осталось`
        : finance.debt > 0 ? `Долг ${money(finance.debt)}` : `Баланс ${money(Math.max(0, finance.balance || 0))}`;
    ...
    }).join('') : '<div class="empty">Ученики не найдены</div>';
  }
  ...
  function renderGroups() {
    ...
    if (!grid) return;
    grid.innerHTML = state.groups.length ? state.groups.map((group) => `<article class="card student-card group-card" data-group="${group.id}" tabindex="0" role="button" aria-label="Редактировать группу ${escapeHtml(group.name)}"><div class="student-head"><div class="avatar">${escapeHtml(initials(group.name))}</div><div><h3>${escapeHtml(group.name)}</h3><div class="sub">${escapeHtml(group.grade || 'Группа')}</div></div><button class="icon-btn quick-delete-group" type="button" data-delete-group="${group.id}" aria-label="Удалить группу">×</button></div><div class="student-card-meta"><span>${escapeHtml(scheduleText(group.scheduleSlots || []))}</span><span>${(group.members || []).length} уч.</span></div><div class="sub">${(group.members || []).map((id) => state.students.find((student) => student.id === id)?.name).filter(Boolean).map(escapeHtml).join(', ') || 'Без участников'}</div></article>`).join('') : '<div class="empty">Групп пока нет</div>';
  }
  ...
  function render() {
    ...
    if (active && $('#profileModal')?.classList.contains('open') && store.getState().students.some((student) => student.id === active)) profile.render(active);
  }
  ...
  $('#studentGrid')?.addEventListener('click', async (event) => {
    ...
    if (deleteId) {
      ...
      if (student && await dialog.ask(`Удалить ученика «${student.name}» вместе с его занятиями и платежами?`, 'Удаление ученика', 'Удалить')) { service.removeStudent(deleteId); toast('Ученик удалён'); }
      return;
    }
  ...
  $('#studentGrid')?.addEventListener('keydown', (event) => {
    ...
    const id = event.target.closest('[data-student]')?.dataset.student;
    if (id) { event.preventDefault(); profile.render(id); }
  });
  $('#groupGrid')?.addEventListener('click', async (event) => {
    ...
    if (deleteId) {
      ...
      if (group && await dialog.ask(`Удалить группу «${group.name}» и все её занятия?`, 'Удаление группы', 'Удалить')) { service.removeGroup(deleteId); toast('Группа удалена'); }
      return;
    }
  ...
  $('#groupGrid')?.addEventListener('keydown', (event) => {
    if (!['Enter', ' '].includes(event.key) || event.target.closest('button')) return;
    const id = event.target.closest('[data-group]')?.dataset.group;
    if (id) { event.preventDefault(); groupForm.openEdit(id); }
  });
  $('#deleteGroup')?.addEventListener('click', async () => {
    ...
    if (group && await dialog.ask(`Удалить группу «${group.name}» и все её занятия?`, 'Удаление группы', 'Удалить')) { service.removeGroup(id); modal.closeAll(); toast('Группа удалена'); }
  });

  return { render, openNewStudent: studentForm.openNew, openStudent: profile.render, editStudent: studentForm.openEdit, openNewGroup: groupForm.openNew, editGroup: groupForm.openEdit };
}

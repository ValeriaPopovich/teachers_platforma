export function createStore(initialState, { validate } = {}) {
  ...
  function replace(nextState, actionName = 'replace') {
    const copy = structuredClone(nextState);
    assertValid(copy, actionName);
    state = copy;
    notify(actionName);
    return state;
  }
  ...
  function notify(actionName) {
    for (const listener of listeners)
      try {
        listener(state, actionName);
      } catch (error) {
        console.error(`store listener failed on "${actionName}":`, error);
      }
  }

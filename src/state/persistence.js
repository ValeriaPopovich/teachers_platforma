...
function envelopeFor(data, now) {
  return { meta: { schemaVersion: CURRENT_SCHEMA_VERSION, updatedAt: now().toISOString() }, data };
}
export function createLocalPersistence({
  ...
  function saveRecovery(data, reason = 'before-replace-import') {
    const recovery = { savedAt: now().toISOString(), reason, data: structuredClone(data) };

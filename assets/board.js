...
if (page) {
  ...
  function showBoardDialog({ title, message, input = false, confirmText = 'Хорошо', value = '' }) {
    ...
    requestAnimationFrame(() =>
      (input ? $('#boardDialogInput') : $('#boardDialogConfirm')).focus(),
    );
  ...
  function point(e) {
    ...
    return {
      x: Math.max(
        0,
        Math.min(canvasLogicalWidth, ((e.clientX - r.left) * canvasLogicalWidth) / r.width),
      ),
      y: Math.max(
        0,
        Math.min(canvasLogicalHeight, ((e.clientY - r.top) * canvasLogicalHeight) / r.height),
      ),
    };
  }
  ...
  function eraseAt(p) {
    ...
    for (let i = objects.length - 1; i >= 0; i--) {
      ...
      if (o.type === 'stroke') {
        ...
        const b = objectBounds(o);
        const pad = 20;
        hit = p.x > b.x - pad && p.x < b.x + b.w + pad && p.y > b.y - pad && p.y < b.y + b.h + pad;
      }
      if (hit) {
        objects.splice(i, 1);
        return true;
  ...
  function renderPages() {
    ...
    page
      .querySelectorAll('[data-board-background]')
      .forEach((button) =>
        button.classList.toggle(
          'active',
          button.dataset.boardBackground === currentPage().background,
        ),
      );
    requestAnimationFrame(() => {
      fitCanvasToWrap();
  ...
  async function importBoards(file) {
    ...
    try {
      ...
      boardAlert(
        'Не удалось прочитать файл. Похоже, он повреждён или это не резервная копия досок.',
      );
      return;
    }
    ...
    setStatus(
      restored
        ? `Восстановлено досок: ${restored}`
        : 'Новых или более свежих досок в копии не найдено',
    );
  ...
  page
    ...
    .forEach((button) => {
      button.addEventListener('mouseenter', () =>
        showBrushOptions(button.dataset.boardTool, button),
      );
  ...
  $('#boardClear').onclick = async () => {
    if (
      await boardConfirm('Очистить всю доску? Содержимое всех страниц будет удалено.', 'Очистить')
    ) {
  ...
  function renderPageForExport(boardPage) {
    ...
    const renderScale = Math.min(4, Math.max(2, 300 / 72));

...
import { pruneOldEvents, sweepOrphans, syncFutureGroupBilling } from '../src/state/maintenance.js';
...
describe('pruneOldEvents', () => {
  it('удаляет events старше retentionDays', () => {
    ...
    const r = pruneOldEvents(data, 45, Date.parse('2026-08-09T00:00:00Z'));
  ...
  it('идемпотентна', () => {
    ...
    const r1 = pruneOldEvents(data);
    const r2 = pruneOldEvents(r1.data);
  ...
  it('не мутирует вход', () => {
    ...
    pruneOldEvents(data, 45, Date.parse('2026-08-09T00:00:00Z'));
    expect(data).toEqual(snap);
  });

  it('никогда не трогает занятия и платежи — это книга учёта', () => {
    const data = seed({
      ...
      payments: [{ id: 'p1', studentId: 's1', date: '2020-01-02', amount: 1000 }],
    });
    const r = pruneOldEvents(data, 45, Date.parse('2026-08-09T00:00:00Z'));
    expect(r.data.lessons).toHaveLength(1);
...
describe('syncFutureGroupBilling', () => {
  it('обновляет только будущие плановые групповые занятия и идемпотентна', () => {
    ...
    expect(once.data.lessons[0]).toMatchObject({ amount: 1500 });
